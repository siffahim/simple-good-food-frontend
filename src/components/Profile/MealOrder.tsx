...
import { useMealOrderHistoryQuery } from '@/redux/apiSlices/orderHistorySlice';
...
const MealOrder = () => {
    ...
    const { data: meals} = useMealOrderHistoryQuery({})
        ...
        <div>
            <table className="w-[1400px] overflow-auto rounded-[5px] rounded-table">
                ...
                <tr className="text-left w-full bg-[#FEE3B8] custom-table-row">
                    {
                        ["S.no ", "Order Type", "OrderDate",  "Price", "Status", "Action"].map((item, index)=>
                    ...
                    meals?.data?.map((item:any, index:number)=>
                        ...
                        <React.Fragment key={index}>
                            <tr>
                                ...
                                <td className='text-center h-[50px] text-[14px] leading-5 text-[#636363] font-medium'> {item?.mealPlanType} </td>

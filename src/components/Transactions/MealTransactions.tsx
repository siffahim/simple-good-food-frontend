...
const MealTransactions = () => {
    ...
    const {data: meals, refetch} = useMealTransactionQuery(page);
    ...
    const handleStatusChange=async(id:any, status:string)=>{
        try {
            await mealTransactionUpdate({id, status}).unwrap().then((result)=>{
                if (result?.success) {
    ...
    return (
        <div>
            <div className='flex items-center justify-between p-4'>
                    <Heading name='Meal Transaction' style='font-medium text-[18px] leading-[24px] text-[#333333] text-center' />
                    <Input
                ...
                <table className="w-full rounded-[5px] rounded-table">
                    <thead>
                        <tr className="text-left w-full bg-[#FEE3B8] custom-table-row">
                            {
                                ["S.no ", "Name", "Order Type", "OrderDate", "Price", "Quantity", "Action"].map((item, index)=>
                    ...
                    <tbody>
                        {
                            meals?.data?.map((item:any, index:number)=>
                                <React.Fragment key={index}>
                                    <tr>
                                        ...
                                        <td className='h-[50px] text-center text-[16px] leading-5 text-[#636363] font-normal'>{item?.mealPlanType}</td>
                                        ...
                                        <td className='h-[50px] text-center text-[16px] leading-5 text-[#636363] font-normal'> {item?.quantity} </td>
                ...
                <div className='my-6 flex items-center justify-center w-full'>
                    <Pagination onChange={(page) => setPage(page.toString())} current={parseInt(page)} showSizeChanger={false} total={meals?.pagination?.total} itemRender={itemRender} />
                </div>
